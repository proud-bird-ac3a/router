import { b0, b1, b2, b3, b4, c0 } from './x1_util.js';
...
async function tls12Prf(secret, label, seed, length, hash = "SHA-256") {
	const labelSeed = concatBytes(b3.encode(label), seed);
...
async function hkdfExpandLabel(hash, secret, label, context, length) {
	const fullLabel = b3.encode("tls13 " + label);
...
function _rL(value, bits) { return (value << bits | value >>> 32 - bits) >>> 0 }

function _cQ(state, indexA, indexB, indexC, indexD) {
...
function _cB(key, counter, nonce) {
...
function T0g(key, nonce, plaintext, additionalData) {
...
function T0h(key, nonce, ciphertext, additionalData) {
...
function T0f(contentType, fragment, version = TLS_VERSION_12) {
	const data = b0(fragment);
...
function T0e(handshakeType, body) { return tlsBytes(handshakeType, (length => [length >> 16 & 255, length >> 8 & 255, 255 & length])(body.length), body) }
class T1 {
	constructor() { this.buffer = new Uint8Array(0) }
	feed(chunk) {
		const bytes = b0(chunk);
...
function T0b(body) {
	...
	if (offset < body.length) {
		...
		for (; offset + 4 <= extensionsEnd;) {
			...
			} else extensionType === EXT_APPLICATION_LAYER_PROTOCOL_NEGOTIATION && extensionLength >= 3 && (alpn = b4.decode(extensionData.slice(3, 3 + extensionData[2])))
		}
	}
...
function T0d(body) {
	...
	for (; offset + 4 <= extensionsEnd;) {
		...
		if (offset += 2, extensionType === EXT_APPLICATION_LAYER_PROTOCOL_NEGOTIATION && extensionLength >= 3) {
			...
			protocolLength > 0 && offset + 3 + protocolLength <= offset + extensionLength && (parsed.alpn = b4.decode(body.slice(offset + 3, offset + 3 + protocolLength)))
		}
...
function T0a(clientRandom, serverName, keyShares, { tls13: enableTls13 = !0, tls12: enableTls12 = !0, alpn = null, chacha = !0 } = {}) {
	const cipherIds = [];
	...
	if (serverName) {
		const serverNameBytes = b3.encode(serverName),
	...
	if (protocols.length) {
		const alpnBytes = concatBytes(...protocols.map((protocol => { const protocolBytes = b3.encode(protocol); return tlsBytes(protocolBytes.length, protocolBytes) })));
...
async function addTurnMessageIntegrity(message, key) {
	...
	return b2(signedMessage, createTurnStunAttribute(TURN_STUN_ATTR.MESSAGE_INTEGRITY, new Uint8Array(signature)));
}

async function readTurnStunMessage(reader, bufferedData = null, timeoutMessage = 'TURN response timed out') {
	let buffer = b1(bufferedData) ? b0(bufferedData) : new Uint8Array(0);
	const pull = async () => {
		...
		if (value?.byteLength) buffer = b2(buffer, value);
	};
...
async function function _wTB(writer, bytes, timeoutMessage) {
	await withTimeout(writer.write(bytes), CONNECT_TIMEOUT_MS, timeoutMessage);
}

export { T0, T1, T2, T0a, T0b, T0c, T0d, T0e, T0f, T0g, T0h };
